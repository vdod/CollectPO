...
#fo 是等一下要用的檔案名稱
fo = os.path.basename(os.path.abspath(srcfolder)) +".txt"

#從 RegexBuddy 來的
reobj = re.compile(r"(?:.*?No\.\s+)(TRL-\d+)(?:PO Ref)(PD\d+)(?:.*?)(\d+\.\d+)TGP(\d+\.\d{2})(?:.*)", re.IGNORECASE | re.MULTILINE)
...
for a,b,c in os.walk(srcfolder.decode('utf-8')):
    for cs in c:
        ...
        if filename.endswith(".pdf"):
            try:
                subject = getPDFContent(filename).encode("ascii", "xmlcharrefreplace")
